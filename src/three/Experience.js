...
export default class Experience {
  ...
  resize() {
    ...
    // The interface sits under the car, so the car is lifted clear of it.
    this.rig.setFramingOffset(this.#isStacked() ? 0.2 : -0.03);
  ...
  #horizontalFill(stacked) {
    ...
    // How much of the rotational diagonal the eye actually sees. A compact
    // upright vehicle hides most of it and can afford 0.707; a car is long and
    // thin, so at a three-quarter yaw its silhouette is nearly the whole
    // diagonal and the same allowance would run it under the copy either side.
    const SILHOUETTE = 0.95;
    // The margin keeps air inside the band — without it the car packs it
    // wall-to-wall and buries the word behind it.
    const band = ((halfBand * 2) / width / SILHOUETTE) * 0.9;
    ...
    return Math.min(0.74, Math.max(0.34, band * squareness));
  }
  ...
  /**
   * A still of what is on screen right now, as a data URL.
   *
   * The card in the corner has no photograph to show — these cars were never
   * photographed — so it borrows a frame from the scene instead. The draw is
   * repeated here rather than read from the last one: outside of a tick the
   * drawing buffer has already been composited and cleared, and reading it
   * would give back an empty canvas.
   */
  captureStill({ width = 560 } = {}) {
    ...
    try {
      ...
      // A lost context is not worth breaking the interface over; the card
      // falls back to holding its own place.
      console.warn('[experience] could not capture a still', error);
      return null;
    }
  }
