...
export default class Lighting {
  constructor({ shadowMapSize = 2048 } = {}) {
    ...
    this.key = new DirectionalLight(new Color('#fff2d6'), 2.4);
    ...
    this.fill = new DirectionalLight(new Color('#8fa6c4'), 0.5);
    this.fill.position.set(5.4, 2.2, 3.2);
    this.group.add(this.fill, this.fill.target);

    this.rim = new DirectionalLight(new Color('#e8c766'), 1.7);
    this.rim.position.set(3.0, 2.4, -5.0);
    this.group.add(this.rim, this.rim.target);

    this.bounce = new HemisphereLight(new Color('#6a6250'), new Color('#0a0b0d'), 0.3);
